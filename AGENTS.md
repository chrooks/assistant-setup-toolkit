...
npm run setup -- --Codex                     # Partial — prompts for mode + behavior
npm run setup -- --Codex --codex --default   # Fully flag-driven, no prompts
...
`canonical/` is the **Canonical Assistant Source** — the single source of truth for all distributable assistant configuration (instructions, skills, hooks, commands). `.codex/` and `.agents/` are **Target Projections** — generated views for Codex CLI that are gitignored and regenerated by the Setup Wizard. `.Codex/` is the repo-local Codex project config (project-scoped skills, rules). Always edit `canonical/` for distributable content; projections follow automatically.
...
| `.Codex/` | Repo-local Codex config (project-scoped skills, rules) |
...
- Use `./scripts/get-skills.sh` when a task needs to download agent skills and create Codex.ai-uploadable zip files.
