...
`canonical/` is the **Canonical Assistant Source** — the single source of truth for all distributable assistant configuration (instructions, skills, hooks, commands, rules). Rules in `canonical/rules/` project to `~/.claude/rules/` and `~/.codex/rules/`. **Target Projections** — generated views for Codex CLI — are staged under the gitignored `.setup/projections/` and regenerated by the Setup Wizard. `.agents/skills/` holds tracked project-scoped Codex skills (Codex discovers repo skills there); `.claude/` is the Claude Code counterpart; repo-root `.codex/` holds only machine-local project config written by hook wiring. Always edit `canonical/` for distributable content; projections follow automatically.
...
| `.agents/skills/` | Tracked project-scoped Codex skills (e.g. `consult`) |
