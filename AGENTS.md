...
`canonical/` is the **Canonical Assistant Source** — the single source of truth for all distributable assistant configuration (instructions, skills, hooks, commands, rules). Rules in `canonical/rules/` project to `~/.claude/rules/` and `~/.codex/rules/`. **Target Projections** — generated views for Codex CLI — are staged under the gitignored `.setup/projections/` and regenerated by the Setup Wizard. `.Codex/` (capital C) is the repo-local Codex project config (project-scoped skills, rules); `.claude/` is the Claude Code counterpart; repo-root `.codex/` holds only project-scoped Codex config written by hook wiring. Always edit `canonical/` for distributable content; projections follow automatically.
...
| `.Codex/` | Repo-local Codex project config (project-scoped skills, e.g. `/consult`) |
...
| `feature_requests/` | ExecPlan docs, PRDs, and design questions for features; gitignored (local-only) |
