...
`canonical/` is the **Canonical Assistant Source** — the single source of truth for all distributable assistant configuration (instructions, skills, hooks, commands, rules). Rules in `canonical/rules/` project to `~/.claude/rules/` and `~/.codex/rules/`. `.codex/` and `.agents/` are **Target Projections** — generated views for Codex CLI that are gitignored and regenerated by the Setup Wizard. `.Codex/` (capital C) is the repo-local Codex project config (project-scoped skills, rules); `.claude/` is the Claude Code counterpart. Always edit `canonical/` for distributable content; projections follow automatically.
