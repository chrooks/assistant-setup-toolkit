...
`canonical/` is the **Canonical Assistant Source** — the single source of truth for all distributable assistant configuration (instructions, skills, hooks, commands, rules). Rules in `canonical/rules/` project to `~/.claude/rules/` and `~/.codex/rules/`. **Target Projections** — generated views for Codex CLI — are staged under the gitignored `.setup/projections/` and regenerated by the Setup Wizard. `.claude/` is the repo-local Claude Code project config (project-scoped skills, rules); `.agents/skills/` holds tracked project-scoped Codex skills; repo-root `.codex/` holds only machine-local project config written by hook wiring. Always edit `canonical/` for distributable content; projections follow automatically.
