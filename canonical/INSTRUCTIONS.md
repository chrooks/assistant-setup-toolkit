...
- **Plain-English first on completion.** Every work-completion report — including subagent result relays — opens with a short what-shipped summary in plain English before any detail; never pass raw agent output through. Case: "simplify this for my human brain" ×7 in one project cycle (2026-07 loop audit).
...
- **Anything a person will touch gets its design frame before it gets built.** `/scope` stamps `surface: true` when the work changes what a user sees or does, and that stamp makes design decisions Meaningful Decisions for the rest of the route — grill resolves them, the plan's acceptance criteria carry them, and a browser drive proves them. New user-facing feature → `/idea-to-design` produces the frame first. `/design-audit` is the after-the-fact companion; reaching for it first means the framing was skipped. Full treatment: the `dev` Skill, "User-facing work: design is a first-class thread, not a finishing coat."
- `/implement` to build · `/verification-loop` after building · `/commit` to commit · `/diagnose` for difficult bugs.
- `/impeccable` for ANY frontend/UI design, implementation, or review work — do not wait to be routed there. Read `~/.claude/rules/web/design-quality.md` first for the quality floor and the reference shelf.
