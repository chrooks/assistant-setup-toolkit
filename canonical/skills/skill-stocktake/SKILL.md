...
This skill audits the **Canonical Assistant Source** — the source of truth, not the
installed projections. The scan scripts default `GLOBAL_DIR` to:

| Path | Description |
|------|-------------|
| `…/toolkit/canonical/skills/` | Canonical Assistant Source (audited by default) |
| `{cwd}/.claude/skills/` | Project-level skills (if the directory exists) |

Auditing canonical means findings translate directly into edits that the Setup Wizard
then projects to all targets. To audit a different tree, set `SKILL_STOCKTAKE_GLOBAL_DIR`.
