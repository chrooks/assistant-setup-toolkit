...
describe("scope Skill", () => {
  it("offers implement instead of trivial and delegates execution to user-level skill guidance", async () => {
    ...
    expect(skill).toMatch(/\/plan.*\/grill-me.*\/implement/is);
    expect(skill).toContain("3. /implement");
    expect(skill).toMatch(/invoke the `implement` Skill/i);
    expect(skill).toContain("Right Skill, Right Job");
    expect(skill).not.toMatch(/\bTrivial\b/);
    expect(skill).not.toMatch(/\btrivial\b/);
  });

  it("projects implement guidance without embedding user-level instruction paths", async () => {
    const skill = await readFile(skillPath, "utf-8");
    const projected = rewriteContentForCodex(skill, true);

    expect(projected).toContain("implement` Skill");
    expect(projected).not.toContain("~/.claude/CLAUDE.md");
    expect(projected).not.toContain("~/.codex/AGENTS.md");
  });

  it("keeps Workflow Triage instructions aligned with implement", async () => {
    const instructions = await readFile(instructionsPath, "utf-8");

    expect(instructions).toContain("## Right Skill, Right Job");
    expect(instructions).toMatch(/`\/scope` it first, then orchestrate the route end-to-end/);
    expect(instructions).toMatch(/grill→plan when it's grillable.*`\/plan`.*`\/implement`/);
